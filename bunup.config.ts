 ...
 * `dist/internal/`. This plugin dynamically rewrites the paths based on depth.
 */
  ...
  hooks: {
    onBuildDone: async ({ files }) => {
      for (const file of files) {
        ...
        // e.g., dist/shared/@outfitter/chunk.js -> 2 levels up to reach dist/
        ...
        const fixed = content
          .replace(/(?<=from\s+")\.\/internal\//g, `${prefix}internal/`)
          .replace(/(?<=import\s*\(")\.\/internal\//g, `${prefix}internal/`)
          .replace(/(?<=import\s+")\.\/internal\//g, `${prefix}internal/`);
