...
`
  ...
  test("transforms single-quoted option and argument declarations", async () => {
    writeTarget(
      "commands/single-quotes.ts",
      `import { Command } from "commander";

export function createSingleQuotesCommand(): Command {
  return new Command('single-quotes')
    .description('Single quote style command')
...
`
    ...
    const updated = readTarget("commands/single-quotes.ts");
    expect(updated).toContain('query: z.string().describe("Search query")');
    expect(updated).toContain('format: z.string().describe("Output format")');
    expect(updated).toMatch(
      /limit: z\.string\(\)\.default\((["'])25\1\)\.describe\("Max results"\)/
    );
    expect(updated).not.toContain(".option('--format <type>'");
    expect(updated).not.toContain(".argument('<query>'");
  });
});
