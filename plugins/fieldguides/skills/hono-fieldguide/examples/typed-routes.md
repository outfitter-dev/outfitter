...
const app = new Hono()
  ...
  .get("/posts", zValidator("query", QuerySchema), (c) => {
    ...
    const total = db.query("SELECT COUNT(*) as count FROM posts").get() as {
      count: number;
    };
...
const res = await client.users[":userId"].posts[":postId"].comments[
  ":commentId"
].$get({
  param: {
    userId: "user-123",
  ...
  .post("/upload", zValidator("form", UploadSchema), async (c) => {
    ...
    if (!allowedTypes[data.category].includes(file.type)) {
      throw new HTTPException(400, {
        message: `Invalid file type for category ${data.category}`,
      });
    }
    ...
    return c.stream(
      async (stream) => {
        ...
        const interval = setInterval(() => {
          stream.writeln(
            `data: ${JSON.stringify({ id: ++id, timestamp: Date.now() })}\n`
          );
        }, 1000);
...
const app = getUserRoute(new Hono()).post("/users", handler);
