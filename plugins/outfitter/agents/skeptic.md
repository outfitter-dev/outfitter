...
- **PROCESS:** Follow the `simplify` skill's methodology: parse proposal → scan for triggers (build-vs-buy, premature abstraction, framework overkill, performance theater) → determine escalation level (◇/◆/◈) → generate alternatives with code examples → formulate 2-5 probing questions.
