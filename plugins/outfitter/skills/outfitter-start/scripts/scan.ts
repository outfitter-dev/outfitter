 ...
 *
 * Scans a codebase for Outfitter adoption candidates and generates
...
function generateScanReport(data: ScanData): string {
  ...
  return render(template, {
    ...
    THROW_FILES: new Set(data.throws.map((t) => t.file)).size,
    TRY_CATCH_COUNT: data.tryCatch.length,
    TRY_CATCH_FILES: new Set(data.tryCatch.map((t) => t.file)).size,
    CONSOLE_COUNT: data.console.length,
    CONSOLE_FILES: new Set(data.console.map((t) => t.file)).size,
    PATH_COUNT: data.paths.length,
    PATH_FILES: new Set(data.paths.map((p) => p.file)).size,
...
async function main() {
  ...
  const outputDir = join(projectRoot, ".agents", "plans", "outfitter-start");
  ...
  console.log("  4. Load outfitter-atlas for patterns and templates");
