...
import type { Daemon, DaemonOptions, DaemonState, ShutdownHandler } from "./types.js";
 ...
 */
async function writePidFile(path: string, pid: number): Promise<Result<void, DaemonError>> {
	try {
...
export function createDaemon(options: DaemonOptions): Daemon {
	...
	async function doStop(): Promise<Result<void, DaemonError>> {
		...
		if (removeResult.isErr()) {
			logger?.error("Failed to remove PID file", { error: removeResult.error.message });
		}
