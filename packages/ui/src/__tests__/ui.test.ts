...
describe("Output Shapes", () => {
	describe("renderTable()", () => {
		...
		it("respects column widths option", () => {
			const data = [{ description: "A very long description that should be truncated" }];
	...
	describe("renderProgress()", () => {
		...
		it("shows percentage", () => {
			const result = renderProgress({ current: 25, total: 100, showPercent: true });
...
describe("Text Formatting", () => {
	describe("wrapText()", () => {
		it("wraps text at specified width", () => {
			const longText = "This is a long sentence that should be wrapped at a specific width";
		...
		it("preserves ANSI codes across line breaks", () => {
			...
			const coloredText = "\x1b[32mThis is green text that needs to wrap\x1b[0m";
