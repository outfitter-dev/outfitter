...
describe("portSchema", () => {
  it("transforms valid port strings to numbers", () => {
    ...
    expect(schema.parse({ PORT: "65535" })).toEqual({ PORT: 65_535 });
  });
...
describe("env export", () => {
  ...
  it("provides typed access to terminal detection variables", () => {
    ...
    expect(
      env.NO_COLOR === undefined || typeof env.NO_COLOR === "boolean"
    ).toBe(true);
    expect(
      env.FORCE_COLOR === undefined || typeof env.FORCE_COLOR === "boolean"
    ).toBe(true);
