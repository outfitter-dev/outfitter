...
import { DEFAULT_PATTERNS, type LogRecord, type Sink, createLogger } from "../index.js";
...
describe("Message String Redaction", () => {
	it("Bearer token in message string is redacted", () => {
		...
		logger.info("Auth header: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.payload.signature");

		expect(records[0].message).not.toContain("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9");
	...
	it("GitHub PAT (ghp_xxx) in message is redacted", () => {
		...
		logger.info("Using token ghp_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8 for auth");

		expect(records[0].message).not.toContain("ghp_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");
		expect(records[0].message).toContain("[REDACTED]");
	});
	...
	it("GitHub OAuth token (gho_xxx) in message is redacted", () => {
		...
		expect(records[0].message).not.toContain("gho_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");
		expect(records[0].message).toContain("[REDACTED]");
	});
	...
	it("GitHub App token (ghs_xxx) in message is redacted", () => {
		...
		expect(records[0].message).not.toContain("ghs_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");
		expect(records[0].message).toContain("[REDACTED]");
	});
	...
	it("GitHub refresh token (ghr_xxx) in message is redacted", () => {
		...
		expect(records[0].message).not.toContain("ghr_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");
		expect(records[0].message).toContain("[REDACTED]");
	});
	...
	it("Multiple patterns in same message are all redacted", () => {
		...
		logger.info("Using Bearer token123.payload.sig and ghp_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");

		expect(records[0].message).not.toContain("Bearer token123");
		expect(records[0].message).not.toContain("ghp_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");
		// Should have two [REDACTED] markers
		const redactedCount = (records[0].message.match(/\[REDACTED\]/g) || []).length;
	...
	it("Non-matching content in message is preserved", () => {
		...
		expect(records[0].message).toBe("User logged in successfully from 192.168.1.1");
	});
});
...
describe("Redaction Edge Cases", () => {
	it("redaction disabled does not redact patterns in message", () => {
		...
		expect(records[0].message).toContain("ghp_A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8");
	});
