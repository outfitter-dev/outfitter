...
describe("Index Creation", () => {
  ...
  it("createIndex rejects invalid table names", () => {
    ...
    expect(() =>
      createIndex({ path: dbPath, tableName: "docs; DROP TABLE users;" })
    ).toThrow();
  });
  ...
  it("createIndex rejects invalid tokenizer values", () => {
    ...
    expect(() =>
      createIndex({
        path: dbPath,
        tokenizer: "bad-tokenizer" as unknown as TokenizerType,
      })
    ).toThrow();
  });
...
describe("Document Operations", () => {
  ...
  it("add() inserts a document into the index", async () => {
    ...
    const db = new Database(dbPath);
    const row = db
      .query("SELECT id, content FROM documents WHERE id = ?")
      .get("doc-1") as {
      id: string;
      content: string;
    } | null;
  ...
  it("addMany() inserts multiple documents in a transaction", async () => {
    ...
    const db = new Database(dbPath);
    const count = db.query("SELECT COUNT(*) as count FROM documents").get() as {
      count: number;
    };
    db.close();
...
describe("Search Operations", () => {
  ...
  it("search() respects offset parameter", async () => {
    ...
    const offsetResults = await index.search({
      query: "JavaScript",
      offset: 1,
      limit: 2,
    });
...
describe("Metadata Operations", () => {
  ...
  it("add() handles complex nested metadata", async () => {
    ...
    if (Result.isOk(result)) {
      expect(result.value.length).toBe(1);
      const meta = result.value[0]?.metadata as unknown as Record<
        string,
        unknown
      >;
  ...
  it("add() handles special characters in metadata", async () => {
    ...
    if (Result.isOk(result)) {
      expect(result.value.length).toBe(1);
      expect(result.value[0]?.metadata?.title).toBe(
        'Special: "quotes" & <tags>'
      );
    }
  });
...
describe("Error Handling", () => {
  ...
  it("add() handles very long content", async () => {
    ...
    const longContent = "word ".repeat(100_000);
