...
export async function createPersistentStore(
  ...
  const flush = async (): Promise<void> => {
    ...
    const tempPath = `${storagePath}.tmp.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2, 8)}`;
...
export function createScopedStore(
  ...
  return {
    ...
    prune(): number {
      // Intentionally prunes expired cursors across all scopes, not just this one.
      // Scoped stores share the same underlying store, and expired means expired
      // regardless of scope — there's no reason to keep stale entries in siblings.
      return store.prune();
    },
