...
import { createRedactor, DEFAULT_PATTERNS, DEFAULT_SENSITIVE_KEYS } from "./redactor.js";
...
export function deserializeError(data: SerializedError): KitError {
	...
	if (!isValidErrorTag(tag)) {
		...
		const props: { message: string; context?: Record<string, unknown> } = { message: data.message };
	...
	switch (tag) {
		case "ValidationError": {
			const props: { message: string; field?: string } = { message: data.message };
		...
		case "RateLimitError": {
			const props: { message: string; retryAfterSeconds?: number } = { message: data.message };
		...
		case "AuthError": {
			const props: { message: string; reason?: "missing" | "invalid" | "expired" } = {
				message: data.message,
			};
			const reason = context["reason"] as "missing" | "invalid" | "expired" | undefined;
			if (reason !== undefined) {
				props.reason = reason;
			}
