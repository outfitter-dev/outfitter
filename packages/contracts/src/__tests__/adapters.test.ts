...
describe("CacheAdapter interface", () => {
	...
	it("compiles with mock implementation", () => {
		const mockCache: CacheAdapter<CachedUser> = {
			...
			async getMany(_keys: string[]): Promise<Result<Map<string, CachedUser>, CacheError>> {
...
describe("AuthAdapter interface", () => {
	it("compiles with mock implementation", () => {
		const mockAuth: AuthAdapter = {
			async get(_key: string): Promise<Result<string | null, AdapterAuthError>> {
				return Result.ok(null);
			},
			async set(_key: string, _value: string): Promise<Result<void, AdapterAuthError>> {
				return Result.ok(undefined);
			},
...
describe("StorageAdapter interface", () => {
	it("compiles with mock implementation", () => {
		const mockStorage: StorageAdapter = {
			...
			async write(_path: string, _data: Uint8Array): Promise<Result<void, StorageError>> {
				return Result.ok(undefined);
			},
