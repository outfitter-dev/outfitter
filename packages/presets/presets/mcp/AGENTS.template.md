...
- **Server not starting in Claude Desktop**: Check that the `command` path in `claude_desktop_config.json` points to the correct `bun` binary (run `which bun` to verify). Use absolute paths.
- **stdio transport errors**: MCP servers use stdin/stdout for communication. Ensure no stray `console.log` calls in server code — use the structured logger instead.
- **Tool not appearing in client**: Verify the tool is registered in `src/mcp.ts` via `server.registerTool()` and the server is restarted.
