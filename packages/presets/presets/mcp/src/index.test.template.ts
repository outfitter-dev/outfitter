...
describe("server", () => {
  ...
  // callTool<T> returns the raw handler output. Pass the expected type
  // parameter to get typed access without manual narrowing.
  test("can call hello tool via harness", async () => {
    const harness = createMcpHarness(server);
    const result = await harness.callTool<{ greeting: string }>("hello", {
      name: "Outfitter",
    });
