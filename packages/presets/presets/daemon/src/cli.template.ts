    ...
    .action(async () => {
      await runHandler({
        ...
        handler: async () => {
          ...
          let response: Response;
          try {
            response = await fetch(`http://unix:${socketPath}:/health`);
          } catch {
            return Result.err(
          ...
          try {
            ...
            return Result.err(
              NetworkError.create("Health endpoint returned invalid JSON")
            );
          }
        },
      });
