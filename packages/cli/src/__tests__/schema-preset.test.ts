...
describe("schema preset edge cases", () => {
  ...
  it("input schema overrides conflicting preset validation for the same field", async () => {
    const preset = createSchemaPreset({
      id: "output",
      schema: z.object({
        format: z
    ...
    cli.register(
      command("run")
        ...
        .input(
          z.object({
            format: z.string().describe("Format from input"),
          })
        )
        .preset(preset)
        .action(async ({ input }) => {
    ...
    await cli.parse(["node", "test", "run", "--format", "yaml"]);
    expect(receivedInput).toEqual({ format: "yaml" });
  });
