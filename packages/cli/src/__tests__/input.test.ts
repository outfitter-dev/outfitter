...
describe("collectIds()", () => {
	...
	test("throws for @file when allowFile: false", async () => {
		...
		try {
			...
			await expect(collectIds(`@${filePath}`, { allowFile: false })).rejects.toThrow();
		} finally {
			await temp.cleanup();
		}
	});
	...
	test("throws for non-existent @file", async () => {
		await expect(collectIds("@/nonexistent/path/ids.txt", { allowFile: true })).rejects.toThrow();
	});

	test("rejects path traversal in @file reference", async () => {
		// Path traversal attempts should be rejected for security
		await expect(collectIds("@../../../etc/passwd", { allowFile: true })).rejects.toThrow(
			/security|traversal|path/i,
		);
		await expect(collectIds("@/tmp/../../../etc/passwd", { allowFile: true })).rejects.toThrow(
			/security|traversal|path/i,
		);
	});
});
...
describe("expandFileArg()", () => {
	...
	test("respects maxSize limit", async () => {
		...
		try {
			...
			await expect(expandFileArg(`@${filePath}`, { maxSize: 100 })).rejects.toThrow();
		} finally {
			await temp.cleanup();
		}
	});
	...
	test("throws for non-existent file", async () => {
		await expect(expandFileArg("@/nonexistent/path/file.txt")).rejects.toThrow();
	});
	...
	test("rejects path traversal in file argument", async () => {
		// Path traversal attempts should be rejected for security
		await expect(expandFileArg("@../../../etc/passwd")).rejects.toThrow(/security|traversal|path/i);
...
describe("parseGlob()", () => {
	...
	test("respects onlyDirectories option", async () => {
		...
		try {
			...
			const result = await parseGlob("*", { cwd: temp.path, onlyDirectories: true });
	...
	test("uses cwd option", async () => {
		...
		try {
			...
			const result = await parseGlob("*.ts", { cwd: join(temp.path, "nested") });
			expect(result).toContain("file.ts");
		} finally {
			await temp.cleanup();
		}
	...
	test("handles followSymlinks option", async () => {
		...
		try {
			...
			const result = await parseGlob("*.ts", { cwd: temp.path, followSymlinks: true });
