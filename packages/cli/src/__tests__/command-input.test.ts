...
describe("CommandBuilder.input()", () => {
  ...
  describe("validation error surfacing", () => {
    ...
    it("does not invoke handler after exitWithError when process.exit is mocked as non-throwing", async () => {
      const cli = createCLI({ name: "test", version: "0.0.1" });
      let handlerCalled = false;
      ...
      let exitCode: number | undefined;
      const originalExit = process.exit;
      // @ts-expect-error - mock process.exit as non-throwing to assert fallthrough behavior
      process.exit = (code?: number) => {
        exitCode = code;
        return undefined;
      };
