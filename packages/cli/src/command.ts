...
function buildMergedSchema(
  ...
  const omitOverriddenPresetFields = (
    ...
    const filteredEntries = Object.entries(schema.shape).filter(
    ...
    const filteredShape = Object.fromEntries(filteredEntries) as z.ZodRawShape;
 ...
 *
 * **Note**: `children` accepts `CommandBuilder` instances, not `Command`. To nest
 * `commandGroup()` results, use `.subcommand()` on a parent builder instead:
 * `command("top").subcommand(command("sub").subcommand(command("leaf").action(h)))`.
 *
 * @example
 * ```typescript
 * program.register(
