 ...
 */
export function mergeSettings(existing: SettingsJson, defaults: SettingsJson): SettingsJson {
...
function mergeHooks(existing: HooksConfig, defaults: HooksConfig): HooksConfig {
	...
	for (const event of events) {
		...
		for (const defaultMatcher of defaultHooks) {
			const existingMatcher = combined.find((m) => m.matcher === defaultMatcher.matcher);
			...
			if (existingMatcher) {
				...
				for (const hook of defaultMatcher.hooks) {
					const isDuplicate = existingMatcher.hooks.some(
						(h) => h.type === hook.type && h.command === hook.command && h.prompt === hook.prompt,
					);
					if (!isDuplicate) {
