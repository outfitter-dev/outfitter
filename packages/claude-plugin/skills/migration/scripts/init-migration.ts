...
// Scanner functions
async function runRg(pattern: string, options: string[] = []): Promise<ScanResult[]> {
...
function suggestErrorMapping(name: string): string {
	...
	if (lower.includes("notfound") || lower.includes("missing")) return "NotFoundError";
	if (lower.includes("validation") || lower.includes("invalid") || lower.includes("input"))
		return "ValidationError";
	if (lower.includes("conflict") || lower.includes("duplicate") || lower.includes("exists"))
		return "ConflictError";
	if (lower.includes("permission") || lower.includes("forbidden")) return "PermissionError";
	if (lower.includes("timeout")) return "TimeoutError";
	if (lower.includes("ratelimit") || lower.includes("rate")) return "RateLimitError";
	if (lower.includes("network") || lower.includes("connection")) return "NetworkError";
	if (lower.includes("auth") || lower.includes("unauthorized") || lower.includes("unauthenticated"))
		return "AuthError";
...
async function scanHandlers(throws: ScanResult[]): Promise<HandlerInfo[]> {
	...
	for (const [file, fileResults] of fileThrows) {
		const funcResults = await runRg("(async )?(function |const )\\w+.*=.*async|async \\w+\\(", [
			file,
		]);

		for (const func of funcResults) {
			const nameMatch = func.content.match(/(function |const )(\w+)|async (\w+)\(/);
			if (nameMatch) {
				const name = nameMatch[2] || nameMatch[3];
				const nearbyThrows = fileResults.filter((t) => Math.abs(t.line - func.line) < 50);
				...
				if (nearbyThrows.length > 0) {
					handlers.push({
						...
						priority: nearbyThrows.length > 3 ? "high" : nearbyThrows.length > 1 ? "medium" : "low",
					});
				}
			}
		}
...
function render(template: string, data: Record<string, unknown>): string {
	...
	result = result.replace(/\{\{#each (\w+)\}\}([\s\S]*?)\{\{\/each\}\}/g, (_, key, content) => {
		...
		return items.map((item) => render(content, item as Record<string, unknown>)).join("");
	});
...
function generateAuditReport(data: ScanData): string {
	const templatePath = join(dirname(import.meta.path), "../templates/audit-report.md");
...
function generatePlanFile(stage: string, data: ScanData): string {
	const templatePath = join(dirname(import.meta.path), `../templates/plan/${stage}`);
	...
	return render(template, {
		...
		DOC_FILES: data.docs.map((f) => ({ file: f, type: "markdown", issues: [], updates: [] })),
...
async function main() {
	...
	// Run scans
	const [throws, tryCatch, consoleLog, paths, errorClasses, docs] = await Promise.all([
		scanThrows(),
		scanTryCatch(),
		scanConsole(),
		scanPaths(),
