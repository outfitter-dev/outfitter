...
3. **Lint**: `bun run lint`
4. **Import verification**: `cd packages/<pkg> && bun -e "import { parseInput } from './src/index.js'"` to verify exports from source
...
- For import verification: `cd packages/<pkg> && bun -e "..."` from the package directory (not repo root)
